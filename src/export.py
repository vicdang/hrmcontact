...
Exports HRM contact list filtered by project to Excel file.
Handles pagination and session management with CAS authentication.
"""
...
def export_contacts(project_id: int, output_file: str = None, base_url: Optional[str] = None, phpsessid: Optional[str] = None, sleep: float = 0.4, force_login: bool = False):
    """
    Main export function.
    
    Args:
        project_id: HRM project ID to export
        output_file: Output Excel file path (uses config default if not provided)
        base_url: Custom base URL (auto-built if not provided)
        phpsessid: Custom PHPSESSID (uses saved session if not provided)
        sleep: Sleep time between requests
        force_login: Force new login, ignore saved session
    """
...
def main():
    ...
    ap.add_argument("--out", default=None, help=f"Output file path (default: {config.get_output_path()})")
